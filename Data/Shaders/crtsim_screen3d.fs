...
void main() {
    ...
    vec3 colorDiff = vec3(0.175, 0.15, 0.2) * diffuse * diffuseBrightness;
    ...
    vec3 colorFres = vec3(0.45, 0.4, 0.5) * fresnel;

    vec4 emissive = sampleCRT(fragTexCoord);
    vec3 color = (colorFres + colorDiff + colorSpec + emissive.rgb) * mix(vec3(1.0), fragColor.rgb, clamp(dimming, 0.0, 1.0));
    color *= brightness;

    finalColor = vec4(clamp(color, 0.0, 1.0), emissive.a) * colDiffuse;
}
