...
void Pattern::circle(std::unordered_set<GObject*> no_dmg, Vector2 center, int nb_bullet, int speed) {
    for (double curve = 0; curve < 360; curve += 360.0 / nb_bullet) {
        ...
        RigidBullet* bullet = (RigidBullet*)Bullet_m::createBullet(RIGID, no_dmg);
        bullet->body_->setSpeed({ tempX, tempY });
        bullet->body_->setCoord(center);
    }
}

void Pattern::line(std::unordered_set<GObject*> no_dmg, Vector2 origin, Vector2 direction, int nb_bullet, int speed) {
